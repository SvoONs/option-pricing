...
Option getOptionFromUserInput() {
    ...
    cin >> right;
    while (!(right == string{"Call"} || right == string{"C"} ||
             right == string{"Put"} || right == string{"P"})) {
    ...
    cin >> style;
    while (!(style == string{"American"} || style == string{"A"} ||
             style == string{"European"} || style == string{"E"})) {
        cout << "Option style must be one of 'American'/'A' or "
                "'European'/'E')\nOption style: ";
        cin >> style;
    }
...
string getModel(Option &option) {
    ...
    if (option.style == OptionStyle::American) {
        ...
        cout << "Model: ";
        cin >> model;
        while (!(model == string{"BlackScholes"} || model == string{"BS"} ||
                 model == string{"MCSimulation"} || model == string{"MC"})) {
            cout << "Model must be one of 'BlackScholes'/'BS' or "
                    "'MCSimulation'/'MC'.\nModel: ";
            cin >> model;
        }
    }
...
void getFairOptionPrice(Option &option, string model) {
    ...
    if (model == "BlackScholes" || model == "BS") {
        ...
        cin >> nPaths;
        while (nPaths < 0) {
            cout << "# of asset price paths to simulate can not be "
